...
**構成要素**:
- **BattleEvaluator**: 各にゃんこについて、賭け金とコストを比較して勝敗を判定
...
- **Player**: さかなの数、獲得したにゃんこ、サイコロ、確認フラグなどを保持
- **RoundResult**: 特定のラウンドの結果（にゃんこ、勝者、賭け金）を保持
- **Bets**: にゃんこへの賭け金情報をカプセル化
- **CatInventory**: 獲得したにゃんこのコレクション管理
- **Cards**: にゃんこカードの種類やコスト、エフェクトなどの静的データ
...
サーバーレス環境におけるリソースの最適化のため、以下の条件でルームドキュメントは自動的に削除されます。

1.  **ホストの退出（ゲスト入室前）**:
    ホストが一人で待機中にルームを退出した場合、そのルームは即座に削除されます。
2.  **両プレイヤーの退出**:
    一方が既に退出（`abandoned: true`）している状態で、もう一方が退出操作を行った場合、ルームは削除されます。
3.  **マッチングのキャンセル**:
    ランダムマッチング中にユーザーがキャンセルした場合、作成されていた一時的なマッチングデータは削除されます。
